...
다시 말해 비동기적 콜백인데, 이것이 바로 **콜백함수를 사용하는 이유**[3]이다. 즉 비동기적으로 처리하되, 특정 작업의 처리가 끝나는 시점에 콜백함수를 실행하기 위해서이다. 이러한 비동기적 콜백의 예로 특정 이벤트가 발생할 때마다 함수를 실행하고 싶은 경우[2] 혹은 ajax를 들 수 있다.
...
2. 콜백 함수란 옵저버 패턴에서 나온 개념으로 객체의 상태 변화(이벤트)가 발생했을 경우 이를 전달하는 함수를 말한다  
	...
	```
3. 쉽게 생각하면, 자바스크립트의 특성으로 인해 콜백의 필요성이 생긴다고 할 수 있다
	* (정확하진 않지만) 비동기, 논블록킹의 특성을 지닌 자바스크립트 언어의 특성은 동기적 처리를 위해 콜백이 필요해진다
	* 그래서 콜백 지옥이라는 게 등장했고..
4. 참고
